...
def parse_packet(msg):
    ...
    if opcode == OPCODE_RRQ:
        ...
        blocknumber = struct.unpack("!H",msg[2:4])[0]
        data = msg[4:]
        return opcode, blocknumber, data
    elif (opcode == OPCODE_ACK):
        blocknr = struct.unpack("!H", msg[2:4])[0]
        data = "" # Used so that the struct always contains three values
...
def tftp_transfer(fd, hostname, direction):
    ...
    finalpacket = False # Boolean for checking if current packet is the final packet of current file
    updated = False
    tosendpacket = ""
    ...
    while True:
        ...
        if (len(readable)< 1) and (opcode != 0):
            if (direction == TFTP_GET) and (finalpacket):
                break
            ...
            if (count == blocknr):
                ...
                fd.write(data)
                client_sock.sendto(tosendpacket,server_address)
                if (size < BLOCK_SIZE): #need to wait in case duplicate of previous packet sent. Might get solved by select()
                    finalpacket = True
